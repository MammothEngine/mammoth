...
func TestRealReplicationScenario(t *testing.T) {
	...
	// Test 5: Node Failover - stop leader and verify new leader election
	t.Run("NodeFailover", func(t *testing.T) {
		// Find current leader
		var oldLeader *testNode
		for _, n := range nodes {
			if n.rs.IsLeader() {
				oldLeader = n
				break
		...
		if oldLeader == nil {
			t.Fatal("No leader found for failover test")
		}
		t.Logf("Current leader: Node %d", oldLeader.id)

		// Stop the leader
		t.Logf("Stopping Node %d (leader)...", oldLeader.id)
		oldLeader.rs.Stop()
		...
		_, _, err := newLeader.rs.Put([]byte("failover_key"), []byte("failover_value"))
		if err != nil {
			t.Fatalf("Failed to write through new leader: %v", err)
		}
		t.Log("Successfully wrote data through new leader")

		// Restart old leader
		t.Logf("Restarting Node %d...", oldLeader.id)
		oldLeader.rs.Start()
	...
	// Test 6: Verify data consistency after failover
	t.Run("DataConsistency", func(t *testing.T) {
		time.Sleep(200 * time.Millisecond)

		for i, n := range nodes {
			status := n.rs.Status()
			t.Logf("Node %d: state=%s term=%d commit=%d",
				i+1, status.State, status.Term, status.CommitIndex)
		}
	})
}
